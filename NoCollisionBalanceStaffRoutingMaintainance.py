...
cycles, filtered_cycles = check() #[[worker_id, [cyc1,cyc2]],[worker_id, [cyc1,cyc2]]]
while len(cycles) > 0:
    prn()
    for u in cycles:#[worker_id, [cyc1,cyc2]]
        print(u)
        print(cycles)
        K,cycles_K=u    #worker_id, [cyc1,cyc2]      
        for cyc in cycles_K:
            if cyc[0]!=0:
...
# shortest_path[i][j] = đường đi ngắng nhất i->j
